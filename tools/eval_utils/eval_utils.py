...
def eval_one_epoch(cfg, model, dataloader, epoch_id, logger, dist_test=False, save_to_file=True, result_dir=None):
    ...
    for i, batch_dict in enumerate(dataloader):
        ...
        # Update recall and GT statistics
    ...
    # Compute total inference time and FPS
    total_time = time.time() - start_time
    num_samples = len(dataloader.dataset)
    fps = num_samples / total_time
    logger.info(f'Total inference time: {total_time:.4f} seconds for {num_samples} samples')
    ...
    # Keep sec_per_example for compatibility
    sec_per_example = total_time / len(dataloader.dataset)
    logger.info('Generate label finished(sec_per_example: %.4f second).' % sec_per_example)

    # In distributed mode, only rank 0 returns results
    if cfg.LOCAL_RANK != 0:
        return {}
